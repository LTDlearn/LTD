...
class My_Server_Redis
{

    private static $cliObj;
    private static $sentinelConfig;
...
//     */
//    public function __construct($isUseCluster=false, $isMaster=true){
//        $this->_isUseCluster = $isUseCluster;
//    }


    public static function getInstance($isUseCluster=false, $isMaster=true){
        if(!isset(self::$cliObj) || empty(self::$cliObj)){
            self::$cliObj = new My_Server_Redis();
        }

        self::$cliObj->_isUseCluster = $isUseCluster;
        self::$sentinelConfig = My_Config_Ini::getDefaultConfigInfo('redis.ini', APPLICATION_ENV);

        if($isMaster){
            self::$cliObj->_linkHandle['master'] = new Redis();
            $redisSentinelClient = new My_Server_RedisSentinelClient(self::$sentinelConfig['master']['host'], self::$sentinelConfig['master']['port']);
            $redisSentinelClient = $redisSentinelClient->get_master_addr_by_name(self::$sentinelConfig['master']['name']);
            $sentinelClientConfig = array();
            $sentinelClientConfig['host'] = array_pop(array_keys($redisSentinelClient[0]));
            $sentinelClientConfig['port'] = $redisSentinelClient[0][$sentinelClientConfig['host']];
            $ret = self::$cliObj->_linkHandle['master']->pconnect($sentinelClientConfig['host'], $sentinelClientConfig['port']);
        }else{
            // 多个 Slave 连接
            self::$cliObj->_linkHandle['slave'][self::$cliObj->_sn] = new Redis();
            $redisSentinelClient = new My_Server_RedisSentinelClient(self::$sentinelConfig['slave']['host'], self::$sentinelConfig['slave']['port']);
            $redisSentinelClient = $redisSentinelClient->get_master_addr_by_name(self::$sentinelConfig['slave']['name']);
            $sentinelClientConfig = array();
            $sentinelClientConfig['host'] = array_pop(array_keys($redisSentinelClient[0]));
            $sentinelClientConfig['port'] = $redisSentinelClient[0][$sentinelClientConfig['host']];
            $ret = self::$cliObj->_linkHandle['slave'][self::$cliObj->_sn]->pconnect($sentinelClientConfig['host'], $sentinelClientConfig['port']);
            ++self::$cliObj->_sn;
        }
        return self::$cliObj;
    }
    ...
    public function getRedis($isMaster=true, $slaveOne=true){
        // 只返回 Master
        if($isMaster){
            $this->_linkHandle['master']->auth(self::$sentinelConfig['master']['auth']);
            return $this->_linkHandle['master'];
        }else{
            $this->_linkHandle['slave']->auth(self::$sentinelConfig['slave']['auth']);
